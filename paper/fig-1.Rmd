...
```{r nfat-validation, fig.width=9, fig.height=6}
nfat <- read_tsv('../data/nfat-luci.tsv')

nfat_fit <- nfat %>% 
    filter(compound %in% c('DMSO', 'aMSH')) %>% 
    drc::drm(
        firefly ~ concentration,
...
nfat %>% 
    filter(compound %in% c('DMSO', 'aMSH')) %>% 
  mutate(concentration = if_else(compound == 'DMSO', 1e-11, concentration)) %>% 
    ggplot(aes(x=concentration, y=firefly)) +
    geom_point() +
    geom_function(fun = nfat_fit$curve[[1]]) +
    scale_x_log10(
      breaks = 10^(seq(-11,-6)),
      labels = c(0, 10^(seq(-10,-6)))
    ) +
  labs(
    x = 'a-MSH Concentration',
    y = 'Luciferase Signal (A.U.)',
    title = 'NFAT Dose Response'
  )
```
