...
```{r amsh-vs-thiq-meta-function, cache=TRUE}
run_metaregression <- function(amsh_level, thiq_level, df){
  
  amsh_vs_thiq_wide <- df %>%
    filter((dose == amsh_level & compound == "aMSH") | (dose == thiq_level & compound == "THIQ")) %>%
  ...
  return(list(resid_data, meta_regression_plot))
}

metareg_low <- run_metaregression(amsh_level = "low", thiq_level = "low", df = cre)
metareg_med <- run_metaregression(amsh_level = "low", thiq_level = "medium", df = cre)
metareg_high <- run_metaregression(amsh_level = "low", thiq_level = "high", df = cre)
```

```{r amsh-vs-thiq-meta, fig.width=6, fig.height=6, cache=TRUE}
metareg_low[[2]] + ggtitle("aMSH Low vs THIQ Low")
metareg_med[[2]] + ggtitle("aMSH Low vs THIQ Medium")
metareg_high[[2]] + ggtitle("aMSH Low vs THIQ High")
```
As a table, here's the 5% FDR's

```{r}
resid_data <- bind_rows(metareg_low[[1]] %>% mutate("level" = "low"),
                        metareg_med[[1]] %>% mutate("level" = "medium"),
                        metareg_high[[1]] %>% mutate("level" = "high"))

sig_vars <- resid_data %>%
  filter(residual_fdr < 0.05) %>%
  mutate(mut_id = str_c(pos, aa)) %>%
  pull(mut_id) %>%
  unique()

resid_data_wide <- resid_data %>%
  filter(str_c(pos, aa) %in% sig_vars) %>%
  select(pos, aa, residual_z, level) %>%
  pivot_wider(names_from = level, values_from = residual_z)
...
compare_amsh_thiq <- function(sel_pos) {
  ...
  cre %>%
    filter(
      pos == sel_pos,
      (dose == "high" & compound == "THIQ") | (dose == "low" & compound == "aMSH")
    ) %>%
    ...
    theme(
      ...
      panel.grid.major.y = element_line(colour = "grey92", size = rel(0.5))
    )
}

compare_129 <- compare_amsh_thiq(121) +
...
```{r pos97-amsh-vs-thiq, fig.width=9, fig.height=4}
compare_97 <- compare_amsh_thiq(97)
...
```{r pos254-amsh-vs-thiq, fig.width=9, fig.height=4}
compare_254 <- compare_amsh_thiq(254)
...
compare_48 <- compare_amsh_thiq(48) +
  scale_y_continuous(breaks = seq(-1.5, 0.5, 0.5), expand = c(0, 0))
