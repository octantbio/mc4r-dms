...
defect_sumstats %>%
    ...
    ggplot() +
        geom_point(aes(x = 2^NoIpsen,
                       y = 2^Ipsen,
                       color = if_else(str_c(pos, mut_aa) %in% sig_vars, "Significant Defect and Rescue, FDR < 0.05", "All Other Variants")),
                   alpha = 0.3) +
        scale_color_manual(values = c("Significant Defect and Rescue, FDR < 0.05" = "red",
                                      "All Other Variants" = "black"),
                           name = "") +
