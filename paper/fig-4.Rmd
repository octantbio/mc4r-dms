...
defect_sumstats %>%
    ...
    ggplot() +
        geom_point(aes(x = 2^NoIpsen,
                       y = 2^Ipsen,
                       color = if_else(str_c(pos, mut_aa) %in% sig_vars, "Joint Significant", "Non-Joint Significant")),
                   alpha = 0.3) +
...
custom_joined_data <- defect_sumstats %>% 
    ...
    inner_join(variant_set, by = join_by(pos, mut_aa)) %>%
    mutate(group = factor(group, levels = c("Responder",
                                            "Non-Responder")))
...
defect_rescue_stats_huang2017 <- defect_sumstats %>%
    dplyr::select(chunk:std.error) %>%
    inner_join(huang2017, by = join_by(pos, mut_aa)) %>%
    mutate(group = factor(group, levels = c("MildLoF",
                                            "PartialLoF",
                                            "FullLoF")))
...
ggplot(defect_rescue_stats) +
    ...
    geom_point(data = defect_rescue_stats_huang2017,
               aes(x = statistic_defect,
                   y = statistic_rescue,
                   color = group),
               size = 3) +
    scale_color_manual(values = c("MildLoF" = "green",
                                  "PartialLoF" = "gold",
                                  "FullLoF" = "red"),
                       name = "Huang 2017")
```
