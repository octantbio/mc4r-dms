...
custom_joined_data <- defect_sumstats %>% 
    ...
    bind_rows(wt_stats) %>%
    mutate(group = factor(group, levels = c("WT",
                                            "Responder",
                                            "Non-Responder")))
...
custom_joined_data %>%
    ...
    ggplot() +
        geom_bar(aes(x = reorder(str_c(pos, aa), as.numeric(pos)),
                     y = 2^log2FoldChange,
                     fill = chaperone),
                 width = 0.5,
                 position = position_dodge(width = 0.5),
                 stat = "identity") +
        geom_errorbar(aes(x = reorder(str_c(pos, aa), pos),
                     y = 2^log2FoldChange,
...
wang2014_barplot_data %>%
    mutate(chaperone = relevel(as.factor(chaperone), ref = "NoIpsen")) %>%
    ggplot() +
        geom_bar(aes(x = reorder(str_c(pos, aa), pos),
                     y = 2^log2FoldChange,
                     fill = chaperone),
                 width = 0.5,
                 position = position_dodge(width = 0.5),
